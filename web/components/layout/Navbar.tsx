...
export function Navbar() {
  ...
  if (pathname.startsWith("/vet")) {
    ...
    contextualLinks = [
      { href: "/vet", label: "Triage", icon: Activity, exact: true },
      { href: "/vet/cases", label: "Cases", icon: ClipboardList },
      { href: "/vet/samples", label: "Samples", icon: FlaskConical },
      { href: "/vet/follow-ups", label: "Visits", icon: Calendar },
      { href: "/vet/profile", label: "Profile", icon: User },
    ];
  } else if (pathname.startsWith("/authority")) {
    currentPortalName = "District Authority";
    contextualLinks = [
      { href: "/authority", label: "Control", icon: Activity, exact: true },
      { href: "/authority/alerts", label: "Alerts", icon: BellRing },
    ...
    contextualLinks = [
      { href: "/farmer", label: "Herd", icon: LayoutDashboard, exact: true },
      { href: "/farmer/report", label: "Report", icon: PlusCircle },
      { href: "/farmer/iot", label: "IoT", icon: Cpu },
      { href: "/farmer/request-help", label: "Agent", icon: UserCheck },
  ...
  return (
    <header className="sticky top-2 sm:top-3 z-50 w-full px-3 sm:px-6 max-w-[1440px] mx-auto mb-3 sm:mb-4 transition-all duration-300">
      <div className="h-14 sm:h-16 px-3 sm:px-4.5 flex items-center justify-between gap-2 rounded-2xl md:rounded-full bg-white/80 dark:bg-[#0A1610]/85 backdrop-blur-[24px] border border-white/80 dark:border-white/14 shadow-[0_8px_32px_rgba(30,58,43,0.06),inset_0_1px_0_rgba(255,255,255,0.9)] dark:shadow-[0_16px_40px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.18)]">
        ...
        <Link href="/" className="flex items-center gap-2 sm:gap-2.5 group shrink-0 select-none">
          <div className="flex h-8.5 w-8.5 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-xl md:rounded-full bg-white/95 dark:bg-white/90 overflow-hidden shadow-[inset_0_1px_rgba(255,255,255,.95),0_2px_8px_rgba(0,0,0,0.12)] border border-black/5 dark:border-white/20 transition-all group-hover:scale-105">
            <Image
              ...
              width={36}
              height={36}
              className="h-full w-full object-contain scale-115"
              priority
            />
          </div>
          <div className="flex items-baseline gap-1.5 shrink-0">
            <span className="font-display text-base sm:text-lg font-bold tracking-tight text-[#15271E] dark:text-[#F4EEE1] whitespace-nowrap">
              Maitri
            </span>
            <span className="hidden sm:inline-block text-[10px] font-sans font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#EAF3EC] dark:bg-[#3F6B4A]/50 text-[#1E3A2B] dark:text-[#BDEEC5] border border-[#1E3A2B]/10 dark:border-white/15 shadow-2xs shrink-0 whitespace-nowrap">
              Intelligence
            </span>
          </div>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1 p-1 rounded-full bg-black/5 dark:bg-black/35 backdrop-blur-md border border-black/5 dark:border-white/10 shrink-0">
              ...
              <Link
                ...
                className={`flex items-center gap-1.5 px-2.5 xl:px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 cursor-pointer whitespace-nowrap ${
                  isActive
                    ...
                    : "text-[#3D5245] hover:text-[#15271E] hover:bg-black/5 dark:text-[#AECEB9] dark:hover:text-[#F4EEE1] dark:hover:bg-white/10"
                }`}
              >
                <Icon className={`h-3.5 w-3.5 shrink-0 ${isActive ? "text-[#8EE6A3] dark:text-[#BDEEC5]" : "text-[#5C7A67] dark:text-[#AECEB9]"}`} />
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <div className="relative" ref={portalRef}>
            ...
            >
              ...
              <ChevronDown className={`h-3 w-3 text-[#4A6B56] dark:text-[#AECEB9] transition-transform shrink-0 ${portalMenuOpen ? "rotate-180" : ""}`} />
            </button>
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3 py-2 rounded-xl text-xs transition-all ${
                        isCurrent
                          ? "bg-[#1E3A2B] text-white dark:bg-[#3F6B4A]/70 dark:text-white font-bold shadow-2xs"
                          : "text-[#15271E] dark:text-[#F4EEE1] hover:bg-black/5 dark:hover:bg-white/10"
                      }`}
                    >
                      <Icon className={`h-4 w-4 ${isCurrent ? "text-[#8EE6A3] dark:text-[#BDEEC5]" : "text-[#B87A1E] dark:text-[#D9A441]"}`} />
                      <div>
                        <div className="font-semibold">{p.label}</div>
                        <div className={`text-[10px] font-normal ${isCurrent ? "text-white/80" : "text-[#5C7A67] dark:text-[#AECEB9]/80"}`}>{p.desc}</div>
                      </div>
                    </Link>
                  );
                })}
          ...
          <div className="flex items-center rounded-full border border-black/8 dark:border-white/15 bg-black/5 dark:bg-[#072417]/60 p-0.5 text-xs font-semibold shadow-[inset_0_1px_rgba(255,255,255,.6)] dark:shadow-[inset_0_1px_rgba(255,255,255,.1)] backdrop-blur-xl shrink-0" role="group" aria-label="Language switcher">
            <button
              type="button"
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              ...
              <div className="shrink-0 flex items-center">
                <UserButton
                  appearance={{
                    elements: {
                      userButtonAvatarBox: "h-8 w-8 sm:h-8.5 sm:w-8.5 border border-[#1E3A2B]/20 dark:border-[#BDEEC5]/60 hover:border-[#1E3A2B] dark:hover:border-[#BDEEC5] transition-all rounded-full shadow-xs",
                    },
                  }}
                />
              </div>
